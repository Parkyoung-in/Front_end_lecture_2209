...
while ( second < first + 4) {
    second = Math.ceil(Math.random()*21 + 10);  // 10 ~ 30
}
console.log(first, second);
const powerArray = [];
for (let i = first; i<=second; i++) {
    if (i == first+1 || i == second-1) // 앞에서 2번째, 뒤에서 2번째 제외
        continue;
...
const models1 = cars.map(x => x.split(' ').slice(1).join(' '));     // 배열
const models2 = cars.map(x => x.substring(x.indexOf(' ')+1));       // 문자열
console.log(models1);
console.log(models2);